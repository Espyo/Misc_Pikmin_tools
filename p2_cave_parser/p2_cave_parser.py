##
#  The purpose of this code is to provide a function, parse_cave_from_file(),
...
#  @param infile Input file.
#  @param cave_data The RawCave cave data object to fill.
        ...
        if searching_start:
            ...
            if line.find('{c000}') != -1:
                ...
                for s in range(sublevel_total):
                    cave_data.sublevels.append(RawSublevel())
...
#  @param infile Input file.
#  @param cave_data The RawCave cave data object to fill.
#  @param sublevel_nr This sublevel's index number.
    ...
    for line in infile:
        ...
        if searching_start:
            ...
            for e in range(entry_count):
                cave_data.sublevels[sublevel_nr].teki_objects.append(RawObject())
            ...
            if next_is_weight:
                ...
                if underscore_pos == -1:
                    ...
                    while underscore_pos != -1:
                        if tentative_internal_name.lower() in constants.OBJECTS:
...
def read_iteminfo(infile, cave_data, sublevel_nr):
    ...
    for line in infile:
        ...
        if searching_start:
            ...
            if entry_count == 0 : return
            for e in range(entry_count):
                cave_data.sublevels[sublevel_nr].item_objects.append(RawObject())
            searching_count = False
...
def read_gateinfo(infile, cave_data, sublevel_nr):
    ...
    for line in infile:
        ...
        if searching_start:
            ...
            if entry_count == 0 : return
            for e in range(entry_count):
                cave_data.sublevels[sublevel_nr].gate_objects.append(RawGate())
            searching_count = False
...
def read_capinfo(infile, cave_data, sublevel_nr):
    ...
    for line in infile:
        ...
        if searching_start:
            ...
            if entry_count == 0 : return
            for e in range(entry_count):
                cave_data.sublevels[sublevel_nr].cap_objects.append(RawObject())
            searching_count = False
